...
TEMA_OSCURO = {
    ...
    "bg_header": "#0a0a0a",
    "bg_list": "#000000",
    "bg_row_normal": "#000000",
    "bg_row_alt": "#0a0a0a",
    "bg_row_active": "#2E0B0E",
    "bg_row_last": "#1A1200",
    "bg_row_hover": "#111111",
    "bg_scrollbar": "#0a0a0a",
    "bg_footer": "#050505",
...
# ---------------------------------------------------------------------------
REPRODUCTORES_MAPA = {
    "mpc-hc.exe": "CODEC MPC",
    "mpc-hc64.exe": "CODEC MPC",
    "mpc-be.exe": "CODEC MPC",
    "mpc-be64.exe": "CODEC MPC",
    "mpc.exe": "CODEC MPC",
    "mediaplayerclassic.exe": "CODEC MPC",
    "k-lite.exe": "CODEC MPC",
    "vlc.exe": "VLC",
    "potplayer.exe": "POTPLAYER",
    "potplayer64.exe": "POTPLAYER",
    "potplayermini.exe": "POTPLAYER",
    "potplayermini64.exe": "POTPLAYER",
    "daum.potplayer.exe": "POTPLAYER",
    "mpv.exe": "MPV",
    "wmplayer.exe": "WINDOWS MEDIA",
    "microsoft.media.player.exe": "PELICULAS Y TV",
    "video.ui.exe": "PELICULAS Y TV",
    "kmplayer.exe": "KMPLAYER",
    "gom.exe": "GOM PLAYER",
    "gomplayerfree.exe": "GOM PLAYER",
...
def esta_archivo_en_uso(ruta):
    ...
    try:
        h = _k32.CreateFileW(
            ruta, GENERIC_READ, 0, None, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, None
        )
        ...
        if namel in t or (len(base) > 3 and base in t):
            ...
            if hp:
                ...
                if _k32.QueryFullProcessImageNameW(hp, 0, img, ctypes.byref(sz)):
                    ...
                    found.append(
                        REPRODUCTORES_MAPA.get(exe, exe.replace(".exe", "").upper())
                    )
                _k32.CloseHandle(hp)
            return False
        return True
...
def _hbitmap_to_ppm_b64(hbm, tw, th):
    hdc = _u32.GetDC(None)
    bmi = (
        struct.pack("<IiiHHIIiiII", 40, tw, -th, 1, 32, 0, 0, 0, 0, 0, 0) + b"\x00" * 4
    )
...
def extract_thumb_bytes(filepath, tw=THUMB_W, th=THUMB_H):
    try:
        ...
        if not cap.isOpened():
            ...
            with open(
                os.path.join(log_dir, "tracker_debug.log"), "a", encoding="utf-8"
            ) as _log:
                _log.write(
                    f"OpenCV Fallo: cap.isOpened() devolvio False para {filepath}\n"
                )
            return None
        ...
        if not ret or frame is None:
            ...
            with open(
                ...
                _log.write(
                    f"OpenCV Fallo: cap.read() devolvio ret={ret} para {filepath} en el frame {target_frame}\n"
                )
            return None
        ...
        with open(
            ...
            _log.write(f"OpenCV Fallo para {filepath}: {e!r}\n")
        return None
...
class Settings:
    ...
    def carpetas_seguidas(self):
        return self.data.get("carpetas_seguidas", [])

    @property
    def carpeta_activa(self):
        return self.data.get("carpeta_activa", "")

    def add_carpeta(self, ruta):
        if not getattr(self, "_is_valid_path", lambda x: True)(ruta):
            return
    ...
    def _load(self):
        ...
        def is_valid(p):
            try:
                ...
                if not p or not isinstance(p, str) or not os.path.isdir(p):
                    return False
        ...
        valid_carpetas = [
            c for c in self.data.get("carpetas_seguidas", []) if is_valid(c)
        ]
    ...
    @property
    def tema(self):
        return self.data.get("tema_actual", "oscuro")

    @tema.setter
    def tema(self, v):
        self.data["tema_actual"] = v

    # ── Vista ─────────────────────────────────────────────────────────────────
    @property
    def vista(self):
        return self.data.get("vista", "lista")
    ...
    # ── Font ──────────────────────────────────────────────────────────────────
    @property
    def fuente_familia(self):
        return self.data["fuente_familia"]

    @fuente_familia.setter
    def fuente_familia(self, v):
        self.data["fuente_familia"] = v

    @property
    def fuente_tamano(self):
        return self.data["fuente_tamano"]

    @fuente_tamano.setter
    def fuente_tamano(self, v):
        self.data["fuente_tamano"] = int(v)
...
class CardView:
    ...
    def __init__(self, parent, app):
        ...
        self._sb = tk.Scrollbar(
            ...
            orient="vertical",
            bg=C["bg_scrollbar"],
            troughcolor=C["bg_list"],
            width=8,
            relief="flat",
            bd=0,
        )
        self._canvas = tk.Canvas(
            self._container, bg=C["bg_list"], highlightthickness=0, bd=0
        )
        ...
        for w in [self._canvas, self._inner]:
            ...
            w.bind(
                "<Button-1>",
                lambda e, wid=w: (
                    self._app._force_focus(-1) if e.widget == wid else None
                ),
            )
    ...
    def update_states(self, ultimo):
        font_cfg = self._app.settings.fuente
        for fn, w in self._cards.items():
            ...
            if fn == ultimo:
                ...
                frm.configure(
                    bg=bg_c, highlightbackground=border_c, highlightthickness=bthick
                )
                title.configure(bg=bg_c, fg=fg_c, font=font_cfg)
                ...
                title.configure(bg=C["bg_card"], fg=C["fg_title"], font=font_cfg)
    ...
    def set_focus(self, idx):
        ...
        if d:
            d["frame"].config(
                highlightbackground=C.get("focus_border", "#ffff00"),
    ...
    def _make_card(self, filename, filepath, cache, dark):
        ...
        r = 22
        tcv.create_rectangle(0, 0, THUMB_W, THUMB_H, fill=C["thumb_bg"], outline="")
        tcv.create_oval(
            cx - r, cy - r, cx + r, cy + r, fill=C["thumb_icon"], outline=C["border"]
        )
        tcv.create_text(
            cx + 2,
            cy,
        ...
        title = tk.Label(
            frm,
            text=truncar(os.path.splitext(filename)[0], 28),
            font=self._app.settings.fuente,  # FIX: uses full bold font from settings
            bg=C["bg_card"],
            fg=C["fg_title"],
            wraplength=self.CARD_W - 14,
            anchor="w",
            justify="left",
        )
        ...
        # ── Status badge ─────────────────────────────────────────────────────
        badge = tk.Label(
            frm, text="", font=("Segoe UI", 8), bg=C["bg_card"], fg=C["fg_sub"]
        )
        ...
        for widget in [frm, tcv, title, badge, num]:
            widget.bind(
                "<Button-1>",
                lambda e, fn=filename: self._app._force_focus(
                    self._app.videos.index(fn)
                    if fn in getattr(self._app, "videos", [])
                    else 0
                ),
            )
            widget.bind(
                "<Double-Button-1>", lambda e, fn=filename: self._app._play_file(fn)
            )
...
class VentanaConfiguracion(tk.Toplevel):
    ...
    def _build(self):
        ...
        tk.Label(
            self,
            text="  Configuracion de Apariencia",
        ...
        self._btn(
            bar, "Guardar y Aplicar", self._save, C["border_active"], "#000"
        ).pack(side="right", padx=(0, 14))
        self._btn(bar, "Cancelar", self.destroy, C["bg_btn"], C["fg_btn"]).pack(
            side="right", padx=6
        )
        ...
        rf.pack(fill="x", **p)
        for val, lbl in [
            ("oscuro", "Modo Oscuro (Catppuccin)"),
            ("claro", "Modo Claro"),
        ]:
            tk.Radiobutton(
                rf,
                text=lbl,
                variable=self._tema_v,
                value=val,
                bg=bg,
                fg=C["fg_normal"],
                selectcolor=C["bg_btn"],
                activebackground=bg,
        ...
        tk.Label(
            body,
            text="  Cada tema conserva sus colores de forma independiente",
            font=("Segoe UI", 8),
            bg=bg,
            fg=C["fg_sub"],
    ...
    def _sec(self, p, t, is_first=False):
        bg = C["bg_header"]
        if not is_first:
            tk.Frame(p, bg=C["separator"], height=1).pack(
                fill="x", padx=20, pady=(10, 0)
            )
        tk.Label(
            p, text=t, font=("Segoe UI", 10, "bold"), bg=bg, fg=C["fg_title"]
        ).pack(anchor="w", padx=20, pady=(4 if not is_first else 0, 2))
    ...
    def _color_row(self, parent, key, label, bg):
        ...
        sw = tk.Label(f, text="      ", bg=col, relief="groove", bd=1, cursor="hand2")
    ...
    def _reload_swatches(self, tema):
        ...
        for sk, sw in self._swatches.items():
            col = self._pending[tema].get(sk, self._st.get_effective_color(sk, tema))
            try:
                sw.configure(bg=col)
            except Exception:
                pass
    ...
    def _refresh_self_theme(self):
        ...
        try:
            ...
            if hasattr(self, "fuente_menu"):
                self.fuente_menu.config(
                    bg=C["bg_btn"],
                    fg=fg_normal,
                    activebackground=C["bg_btn_hover"],
                    activeforeground=fg_normal,
                )
                self.fuente_menu["menu"].config(
...
class MosaicosView:
    ...
    def _relayout(self):
        ...
        for idx, fn in enumerate(self._order):
            if fn in self._tiles:
                r = idx // self._ncols
                c = idx % self._ncols
                self._tiles[fn]["frame"].grid(
                    row=r, column=c, padx=6, pady=6, sticky="ew"
                )
    ...
    def rebuild(self, videos, dirpath, ult, cache):
        ...
        for idx, fn in enumerate(self._order):
            tf = tk.Frame(
                self._inner, bg=C["bg_card"], bd=3, relief="flat", cursor="hand2"
            )

            # Left: Thumbnail canvas 100x56
            tcv = tk.Canvas(
                tf, width=100, height=56, bg=C["thumb_bg"], highlightthickness=0, bd=0
            )
            ...
            title = tk.Label(
                rf,
                text=f" {idx + 1:>3}   {truncar(fn, 50)}",
                font=self._app.settings.fuente,
                bg=C["bg_card"],
                fg=C["fg_normal"],
                anchor="w",
            )
            ...
            for w in (tf, tcv, rf, title, status):
                w.bind(
                    "<Button-1>",
                    lambda e, f=fn: self._app._force_focus(
                        self._order.index(f) if f in self._order else 0
                    ),
                )
    ...
    def update_states(self, ult):
        ...
        for fn, d in self._tiles.items():
            ...
            if fn == ult:
                tf.config(
                    bg=C["border"],
                    highlightbackground=C["fg_last"],
                    highlightthickness=2,
                )
                rf.config(bg=C["bg_card_last"])
                title.config(bg=C["bg_card_last"], fg=C["fg_last"], font=font_cfg)
                status.config(
                    bg=C["bg_card_last"], fg=C["fg_last"], text="[✔ ULTIMO VISTO]"
                )
            else:
                tf.config(
                    bg=C["border_card"],
                ...
                title.config(bg=C["bg_card"], fg=C["fg_normal"], font=font_cfg)
                status.config(bg=C["bg_card"], text="")

    def _on_hover(self, fn):
        if fn not in self._tiles or fn == self._app.ultimo_visto:
            return
        d = self._tiles[fn]
        d["rf"].config(bg=C["bg_card_hover"])
        d["title"].config(bg=C["bg_card_hover"])
    ...
    def _on_leave(self, fn):
        if fn not in self._tiles or fn == self._app.ultimo_visto:
            return
        d = self._tiles[fn]
        d["rf"].config(bg=C["bg_card"])
        d["title"].config(bg=C["bg_card"])
...
class VideoTrackerApp:
    ...
    def cambiar_carpeta_activa(self, nueva_ruta):
        ...
        if not nueva_ruta or not os.path.isdir(nueva_ruta):
            ...
            self.videos = []
            self.ultimo_visto = ""
            if hasattr(self, "_card_view") and self._card_view:
                self._card_view.clear()
            if hasattr(self, "_mos_view") and self._mos_view:
                self._mos_view.clear()
    ...
    def _show_empty_state(self):
        ...
        if (
            getattr(self, "_fm_frame", None)
            and self._fm_frame
            and self._fm_frame.winfo_exists()
        ):
        ...
        lbl = tk.Label(
            frm_center,
            text="No hay ninguna carpeta configurada.",
            bg=C["bg_root"],
            fg=C["fg_sub"],
            font=("Segoe UI", 12),
        )
        ...
        btn_add = tk.Label(
            frm_center,
            text=" Seleccionar carpeta a traquear ",
            bg=C["border_active"],
            fg="#fff",
            font=("Segoe UI", 12, "bold"),
            cursor="hand2",
    ...
    def _toggle_folder_manager(self):
        ...
        tk.Label(
            hdr,
            text="Gestor de Trackers",
            font=("Segoe UI", 14, "bold"),
            bg=C["bg_header"],
            fg=C["fg_title"],
        ).pack(side="left", pady=15, padx=10)

        # Add button (Prominent)
        btn_add = tk.Label(
            self._fm_frame,
            text=" + Añadir carpeta a traquear ",
            bg=C["border_active"],
            fg="#ffffff",
            font=("Segoe UI", 10, "bold"),
            cursor="hand2",
    ...
    def _draw_folder_list(self):
        ...
        if not carpetas:
            ...
            for c in carpetas:
                ...
                row.bind(
                    "<Button-1>",
                    lambda e, ruta=c: self._activate_folder_from_manager(ruta),
                )
                lbl.bind(
                    "<Button-1>",
                    lambda e, ruta=c: self._activate_folder_from_manager(ruta),
                ...
                make_btn(
                    row, "X", "#DC143C", lambda p=c: self._remove_folder_from_manager(p)
                )
                ...
                if not is_active:
                    make_btn(
                        ...
                        "Activar",
                        C["border_active"],
                        lambda p=c: self._activate_folder_from_manager(p),
                    )
                else:
                    tk.Label(
                        row,
    ...
    def _remove_folder_from_manager(self, ruta):
        ...
        if messagebox.askyesno(
            "Confirmar", f"¿Dejar de traquear esta carpeta?\n{ruta}"
        ):
    ...
    def _build_header(self):
        ...
        for w in (self._btn_v_list, self._btn_v_mos, self._btn_v_tarj):
            w.bind(
                "<Enter>",
                lambda e, w=w: (
                    w.configure(bg=C["bg_btn_hover"])
                    if w.cget("bg") != C["border_active"]
                    else None
                ),
            )
            w.bind(
        ...
        self._btns = [b1, b2, b3]
        self._v_btns = {
            "lista": self._btn_v_list,
            "mosaicos": self._btn_v_mos,
            "tarjetas": self._btn_v_tarj,
        }
        self._update_v_btns()
    ...
    def _build_content(self):
        ...
        lc.bind(
            "<Button-1>",
            lambda e, w=lc: self._force_focus(-1) if e.widget == w else None,
        )
        lf.bind(
            "<Button-1>",
            lambda e, w=lf: self._force_focus(-1) if e.widget == w else None,
        )
        ...
        self.listbox.bind(
            "<ButtonRelease-1>",
            lambda e: (
                self._force_focus(self.listbox.curselection()[0])
                if self.listbox.curselection()
                else self._force_focus(-1)
            ),
        )
    ...
    def _build_footer(self):
        ...
        self.lbl_total = tk.Label(
            ft, text="", font=("Segoe UI", 8, "bold"), bg=C["bg_footer"], fg=C["fg_sub"]
        )
    ...
    def _rebuild_cards(self):
        ...
        if self._vista_actual == "tarjetas":
            self._card_view.rebuild(
                self.videos, self.directorio, self.ultimo_visto, self._thumb_cache
            )
        elif self._vista_actual == "mosaicos":
            self._mos_view.rebuild(
                self.videos, self.directorio, self.ultimo_visto, self._thumb_cache
            )
    ...
    def _row_style(self, idx):
        ...
        if idx == self._focus_index:
            self.listbox.itemconfig(
                idx, bg=C.get("focus_border", "#ffff00"), fg=C["bg_root"]
            )
            return
        v = self.videos[idx]
    ...
    def _update_ui(self):
        ...
        if getattr(self, "_empty_state_frame", None):
            try:
                if self._empty_state_frame.winfo_exists() and self.directorio and self.videos:
                    self._empty_state_frame.place_forget()
                    self._empty_state_frame.pack_forget()
            except Exception:
                pass
    ...
    def _update_list(self):
        ...
        for idx, v in enumerate(self.videos):
            ...
            if v == self.ultimo_visto:
                idx_uv = idx
                self.listbox.insert(tk.END, f" {num}  \u2714  {nc:<56} [ULTIMO VISTO]")
                self.listbox.itemconfig(idx, bg=C["bg_row_last"], fg=C["fg_last"])
            else:
    ...
    def _update_labels(self):
        ...
        uv_idx = (
            self.videos.index(self.ultimo_visto)
            if self.ultimo_visto in self.videos
            else -1
        )
        if uv_idx >= 0:
        ...
        try:
            ...
            for v in self.videos:
                if esta_archivo_en_uso(os.path.join(self.directorio, v)):
                    activo = v
                    break
            if activo and self.ultimo_visto != activo:
    ...
    def _apply_focus(self):
        ...
        if self._vista_actual == "lista":
            ...
            for i in range(len(self.videos)):
                if i == idx:
                    self.listbox.itemconfig(
                        i, bg=C.get("focus_border", "#ffff00"), fg=C["bg_root"]
                    )
                else:
                    if self.videos[i] == self.ultimo_visto:
                        ...
                        self.listbox.itemconfig(
                            i,
                            bg=C["bg_row_alt"] if i % 2 else C["bg_row_normal"],
                            fg=C["fg_normal"],
                        )
        elif self._vista_actual == "mosaicos" and self._mos_view:
    ...
    def _apply_theme_all(self):
        ...
        if (
            getattr(self, "_empty_state_frame", None)
        ...
        if self._mos_view:
            self._mos_view.apply_theme()

        # FIX: Immediately rebuild the active view with new colors/font
        # En vez de destruir y recrear (lo cual limpia todo y rompe las miniaturas),
        # delegamos a _update_ui() que repinta el color de las tarjetas activas.
        self._update_ui()
